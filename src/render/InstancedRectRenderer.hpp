...
class CInstancedRectRenderer {
  public:
    ...
    size_t getBatchSize() const { return m_instanceData.size() / FLOATS_PER_INSTANCE; }
    
    // Performance metrics
    size_t getMaxInstancesPerDraw() const { return MAX_INSTANCES_PER_DRAW; }
    bool   isInstancedRenderingSupported() const { return m_supported; }
