...
void IHyprLayout::onMouseMove(const Vector2D& mousePos) {
    ...
    if (g_pInputManager->dragMode == MBIND_MOVE) {
        ...
        if (DRAGGINGWINDOW->m_bIsFloating) {
            ...
            Vector2D MINSIZE = g_pXWaylandManager->getMinSizeForWindow(DRAGGINGWINDOW)
                                   .clamp({DRAGGINGWINDOW->m_sAdditionalConfigData.minSize.toUnderlying().x, DRAGGINGWINDOW->m_sAdditionalConfigData.minSize.toUnderlying().y});
            Vector2D MAXSIZE = g_pXWaylandManager->getMaxSizeForWindow(DRAGGINGWINDOW)
                                   .clamp({}, {DRAGGINGWINDOW->m_sAdditionalConfigData.maxSize.toUnderlying().x, DRAGGINGWINDOW->m_sAdditionalConfigData.maxSize.toUnderlying().y});
