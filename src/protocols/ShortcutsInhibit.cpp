...
bool CKeyboardShortcutsInhibitProtocol::isInhibited() {
    ...
    const PHLWINDOW PWINDOW = g_pCompositor->getWindowFromSurface(g_pCompositor->m_pLastFocus.lock());
    if (PWINDOW && PWINDOW->m_sWindowData.noShortcutsInhibit.valueOrDefault())
        return false;
