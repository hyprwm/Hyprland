...
CConfigManager::CConfigManager() {
    ...
    m_pConfig->addSpecialCategory("monitorv2", {.key = "output"});
    m_pConfig->addSpecialConfigValue("monitorv2", "disabled", Hyprlang::INT{0});
    m_pConfig->addSpecialConfigValue("monitorv2", "mode", {"preferred"});
    m_pConfig->addSpecialConfigValue("monitorv2", "position", {"auto"});
    m_pConfig->addSpecialConfigValue("monitorv2", "scale", Hyprlang::FLOAT{1.0});
    m_pConfig->addSpecialConfigValue("monitorv2", "addreserved", {STRVAL_EMPTY});
    m_pConfig->addSpecialConfigValue("monitorv2", "mirror", {STRVAL_EMPTY});
    m_pConfig->addSpecialConfigValue("monitorv2", "bitdepth", {STRVAL_EMPTY}); // TODO use correct type
    m_pConfig->addSpecialConfigValue("monitorv2", "cm", {"auto"});
    m_pConfig->addSpecialConfigValue("monitorv2", "sdrbrightness", Hyprlang::FLOAT{1.0});
    m_pConfig->addSpecialConfigValue("monitorv2", "sdrsaturation", Hyprlang::FLOAT{1.0});
    m_pConfig->addSpecialConfigValue("monitorv2", "vrr", Hyprlang::INT{0});
    m_pConfig->addSpecialConfigValue("monitorv2", "transform", {STRVAL_EMPTY}); // TODO use correct type
...
Hyprlang::CParseResult CConfigManager::handleMonitorv2() {
    ...
    for (const auto& output : m_pConfig->listKeysForSpecialCategory("monitorv2")) {
        ...
        if (VAL && VAL->m_bSetByUser && std::any_cast<Hyprlang::INT>(VAL->getValue()))
            parser.setDisabled();
        VAL = m_pConfig->getSpecialConfigValuePtr("monitorv2", "mode", output.c_str());
        if (VAL && VAL->m_bSetByUser)
            parser.parseMode(std::any_cast<Hyprlang::STRING>(VAL->getValue()));
        VAL = m_pConfig->getSpecialConfigValuePtr("monitorv2", "position", output.c_str());
        if (VAL && VAL->m_bSetByUser)
            parser.parsePosition(std::any_cast<Hyprlang::STRING>(VAL->getValue()));
        VAL = m_pConfig->getSpecialConfigValuePtr("monitorv2", "scale", output.c_str());
        if (VAL && VAL->m_bSetByUser)
            parser.rule().scale = std::any_cast<Hyprlang::FLOAT>(VAL->getValue());
        VAL = m_pConfig->getSpecialConfigValuePtr("monitorv2", "addreserved", output.c_str());
        if (VAL && VAL->m_bSetByUser) {
        ...
        VAL = m_pConfig->getSpecialConfigValuePtr("monitorv2", "mirror", output.c_str());
        if (VAL && VAL->m_bSetByUser)
            parser.setMirror(std::any_cast<Hyprlang::STRING>(VAL->getValue()));
        VAL = m_pConfig->getSpecialConfigValuePtr("monitorv2", "bitdepth", output.c_str());
        if (VAL && VAL->m_bSetByUser)
            parser.parseBitdepth(std::any_cast<Hyprlang::STRING>(VAL->getValue()));
        VAL = m_pConfig->getSpecialConfigValuePtr("monitorv2", "cm", output.c_str());
        if (VAL && VAL->m_bSetByUser)
            parser.parseCM(std::any_cast<Hyprlang::STRING>(VAL->getValue()));
        VAL = m_pConfig->getSpecialConfigValuePtr("monitorv2", "sdrbrightness", output.c_str());
        if (VAL && VAL->m_bSetByUser)
            parser.rule().sdrBrightness = std::any_cast<Hyprlang::FLOAT>(VAL->getValue());
        VAL = m_pConfig->getSpecialConfigValuePtr("monitorv2", "sdrsaturation", output.c_str());
        if (VAL && VAL->m_bSetByUser)
            parser.rule().sdrSaturation = std::any_cast<Hyprlang::FLOAT>(VAL->getValue());
        VAL = m_pConfig->getSpecialConfigValuePtr("monitorv2", "vrr", output.c_str());
        if (VAL && VAL->m_bSetByUser)
            parser.rule().vrr = std::any_cast<Hyprlang::INT>(VAL->getValue());
        VAL = m_pConfig->getSpecialConfigValuePtr("monitorv2", "transform", output.c_str());
        if (VAL && VAL->m_bSetByUser)
...
std::optional<std::string> CConfigManager::handleMonitor(const std::string& command, const std::string& args) {
    ...
    if (ARGS[1] == "disable" || ARGS[1] == "disabled" || ARGS[1] == "addreserved" || ARGS[1] == "transform") {
        ...
        std::erase_if(m_vMonitorRules, [&](const auto& other) { return other.name == parser.name(); });

        m_vMonitorRules.push_back(parser.rule());
