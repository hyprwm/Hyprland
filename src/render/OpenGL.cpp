...
CHyprOpenGLImpl::CHyprOpenGLImpl() : m_drmFD(g_pCompositor->m_drmRenderNode.fd >= 0 ? g_pCompositor->m_drmRenderNode.fd : g_pCompositor->m_drm.fd) {
    ...
    Log::logger->log(Log::INFO, "MRT support: {} (colorAttachments={}, drawBuffers={})", m_mrtSupported ? "yes" : "no", maxColorAttachments, maxDrawBuffers);
...
void CHyprOpenGLImpl::begin(PHLMONITOR pMonitor, const CRegion& damage_, CFramebuffer* fb, std::optional<CRegion> finalDamage) {
    ...
    if (ENABLE_CAPTURE_MRT) {
        if (m_renderData.pCurrentMonData->offloadFB.ensureSecondColorAttachment()) {
            ...
            Log::logger->log(Log::WARN, "Failed to ensure second color attachment; disabling capture MRT for this frame");
...
bool CHyprOpenGLImpl::initShaders() {
    ...
    try {
        ...
        try {
            ...
            Log::logger->log(Log::WARN, "capture.glsl missing from assets, falling back to inline macro only: {}", e.what());
            if (m_mrtSupported)
