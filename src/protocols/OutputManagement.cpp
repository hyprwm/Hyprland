...
COutputManager::COutputManager(SP<CZwlrOutputManagerV1> resource_) : m_resource(resource_) {
    ...
    LOG(Log::DEBUG, "New OutputManager registered");
    ...
    m_resource->setCreateConfiguration([this](CZwlrOutputManagerV1* r, uint32_t id, uint32_t serial) {
        LOG(Log::DEBUG, "Creating new configuration");
    ...
    for (auto const& m : State::monitorState()->allMonitors()) {
        LOG(Log::DEBUG, " | sending output head for {}", m->m_name);
    ...
    if (m_monitor->m_enabled) {
        for (auto const& mw : m_modes) {
            ...
            if (m->m_mode == m_monitor->m_output->state->state().mode) {
                if (m->m_mode)
                    LOG(Log::DEBUG, "  | sending current mode for {}: {}x{}@{}", m_monitor->m_name, m->m_mode->pixelSize.x, m->m_mode->pixelSize.y, m->m_mode->refreshRate);
                else
                    LOG(Log::DEBUG, "  | sending current mode for {}: null (fake)", m_monitor->m_name);
...
void COutputMode::sendAllData() {
    ...
    LOG(Log::DEBUG, "  | sending mode {}x{}@{}mHz, pref: {}", m_mode->pixelSize.x, m_mode->pixelSize.y, m_mode->refreshRate, m_mode->preferred);
...
COutputConfiguration::COutputConfiguration(SP<CZwlrOutputConfigurationV1> resource_, SP<COutputManager> owner_) : m_resource(resource_), m_owner(owner_) {
    ...
    m_resource->setEnableHead([this](CZwlrOutputConfigurationV1* r, uint32_t id, wl_resource* outputHead) {
        ...
        if (!HEAD) {
            LOG(Log::ERR, "No head in setEnableHead??");
            return;
        }
        ...
        if (!PMONITOR) {
            LOG(Log::ERR, "No monitor in setEnableHead??");
            return;
        }
        ...
        LOG(Log::DEBUG, "enableHead on {}. For now, doing nothing. Waiting for apply().", PMONITOR->m_name);
    });
    ...
    m_resource->setDisableHead([this](CZwlrOutputConfigurationV1* r, wl_resource* outputHead) {
        ...
        if (!HEAD) {
            LOG(Log::ERR, "No head in setDisableHead??");
            return;
        }
        ...
        if (!PMONITOR) {
            LOG(Log::ERR, "No monitor in setDisableHead??");
            return;
        }

        LOG(Log::DEBUG, "disableHead on {}", PMONITOR->m_name);
...
bool COutputConfiguration::applyTestConfiguration(bool test) {
    if (test) {
        LOG(Log::WARN, "TODO: STUB: applyTestConfiguration for test not implemented, returning true.");
        return true;
    }

    LOG(Log::DEBUG, "Applying configuration");

    if (!m_owner) {
        LOG(Log::ERR, "applyTestConfiguration: no owner?!");
        return false;
    }
    ...
    for (auto const& headw : m_heads) {
        ...
        LOG(Log::DEBUG, "Saving config for monitor {}", PMONITOR->m_name);
        ...
        if (head->m_state.committedProperties & eWlrOutputCommittedProperties::OUTPUT_HEAD_COMMITTED_MODE) {
            ...
            LOG(Log::DEBUG, " > Mode: {:.0f}x{:.0f}@{}mHz", newState.resolution.x, newState.resolution.y, newState.refresh);
            ...
            LOG(Log::DEBUG, " > Custom mode: {:.0f}x{:.0f}@{}mHz", newState.resolution.x, newState.resolution.y, newState.refresh);
        }
        ...
        if (head->m_state.committedProperties & eWlrOutputCommittedProperties::OUTPUT_HEAD_COMMITTED_POSITION) {
            ...
            LOG(Log::DEBUG, " > Position: {:.0f}, {:.0f}", head->m_state.position.x, head->m_state.position.y);
        }
        ...
        if (head->m_state.committedProperties & eWlrOutputCommittedProperties::OUTPUT_HEAD_COMMITTED_ADAPTIVE_SYNC) {
            ...
            LOG(Log::DEBUG, " > vrr: {}", newState.adaptiveSync);
        }
        ...
        if (head->m_state.committedProperties & eWlrOutputCommittedProperties::OUTPUT_HEAD_COMMITTED_SCALE) {
            ...
            LOG(Log::DEBUG, " > scale: {:.2f}", newState.scale);
        }
        ...
        if (head->m_state.committedProperties & eWlrOutputCommittedProperties::OUTPUT_HEAD_COMMITTED_TRANSFORM) {
            ...
            LOG(Log::DEBUG, " > transform: {}", (uint8_t)newState.transform);
        }
    ...
    LOG(Log::DEBUG, "Saved configuration");
...
COutputConfigurationHead::COutputConfigurationHead(SP<CZwlrOutputConfigurationHeadV1> resource_, PHLMONITOR pMonitor_) : m_resource(resource_), m_monitor(pMonitor_) {
    ...
    m_resource->setSetMode([this](CZwlrOutputConfigurationHeadV1* r, wl_resource* outputMode) {
        ...
        if (!MODE || !MODE->getMode()) {
            LOG(Log::ERR, "No mode in setMode??");
            return;
        }

        if (!m_monitor) {
            LOG(Log::ERR, "setMode on inert resource");
            return;
        }
        ...
        LOG(Log::DEBUG, " | configHead for {}: set mode to {}x{}@{}", m_monitor->m_name, MODE->getMode()->pixelSize.x, MODE->getMode()->pixelSize.y, MODE->getMode()->refreshRate);
    });

    m_resource->setSetCustomMode([this](CZwlrOutputConfigurationHeadV1* r, int32_t w, int32_t h, int32_t refresh) {
        if (!m_monitor) {
            LOG(Log::ERR, "setCustomMode on inert resource");
            return;
        }
        ...
        if (refresh == 0) {
            LOG(Log::DEBUG, " | configHead for {}: refreshRate 0, using old refresh rate of {:.2f}Hz", m_monitor->m_name, m_monitor->m_refreshRate);
        ...
        LOG(Log::DEBUG, " | configHead for {}: set custom mode to {}x{}@{}", m_monitor->m_name, w, h, refresh);
    });
    ...
    m_resource->setSetPosition([this](CZwlrOutputConfigurationHeadV1* r, int32_t x, int32_t y) {
        ...
        LOG(Log::DEBUG, " | configHead for {}: set pos to {}, {}", m_monitor->m_name, x, y);
    });
    ...
    m_resource->setSetTransform([this](CZwlrOutputConfigurationHeadV1* r, int32_t transform) {
        ...
        LOG(Log::DEBUG, " | configHead for {}: set transform to {}", m_monitor->m_name, transform);
    });
    ...
    m_resource->setSetScale([this](CZwlrOutputConfigurationHeadV1* r, wl_fixed_t scale_) {
        ...
        LOG(Log::DEBUG, " | configHead for {}: set scale to {:.2f}", m_monitor->m_name, scale);
    });
    ...
    m_resource->setSetAdaptiveSync([this](CZwlrOutputConfigurationHeadV1* r, uint32_t as) {
        ...
        LOG(Log::DEBUG, " | configHead for {}: set adaptiveSync to {}", m_monitor->m_name, as);
    });
}
...
void COutputManagementProtocol::sendPendingSuccessEvents() {
    ...
    LOG(Log::DEBUG, "Sending {} pending configuration success events", m_pendingConfigurationSuccessEvents.size());
