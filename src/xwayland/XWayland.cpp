...
CXWayland::CXWayland(const bool wantsEnabled) {
    ...
    if (!wantsEnabled) {
        NDebug::log(LOG, "XWayland has been disabled, cleaning up...");
    ...
    if (!NFsUtils::executableExistsInPath("Xwayland")) {
        ...
        NDebug::log(LOG, "Unable to find XWayland; not starting it.");
        return;
    }

    NDebug::log(LOG, "Starting up the XWayland server");

    pServer = makeUnique<CXWaylandServer>();

    if (!pServer->create()) {
        NDebug::log(ERR, "XWayland failed to start: it will not work.");
        return;
    }

    m_enabled = true;
#else
    NDebug::log(LOG, "Not starting XWayland: disabled at compile time");
#endif
}
...
#ifndef NO_XWAYLAND
    if (!pWM) {
        NDebug::log(ERR, "Couldn't set XCursor: no XWM yet");
        return;
    }
