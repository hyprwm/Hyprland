...
std::string CCommandFormatter::getMonitorData(PHLMONITOR m, eHyprCtlOutputFormat format) {
    ...
    "hardwareDetails": {{
        ...
        "vrrCapable": {}
            ...
            (m->m_activeWorkspace ? Workspace::workspaceTypeToString(m->m_activeWorkspace->type()) : ""),
            ...
            (m->m_activeSpecialWorkspace ? Workspace::workspaceTypeToString(m->m_activeSpecialWorkspace->type()) : ""),
        ...
        result += std::format(
            ...
            m->m_output->make, m->m_output->model, sc<int>(m->m_output->physicalSize.x), sc<int>(m->m_output->physicalSize.y), m->m_output->serial,
            (m->m_activeWorkspace ? m->m_activeWorkspace->addressableName() : ""), (!m->m_activeWorkspace ? "" : m->m_activeWorkspace->displayName()),
            (m->m_activeSpecialWorkspace ? m->m_activeSpecialWorkspace->addressableName() : ""), (m->m_activeSpecialWorkspace ? m->m_activeSpecialWorkspace->displayName() : ""),
            ...
            sc<int>(m->m_transform), yn(m == Desktop::focusState()->monitor()), sc<int>(m->m_dpmsStatus), m->m_output->state->state().adaptiveSync,
...
std::string CCommandFormatter::getWindowData(PHLWINDOW w, eHyprCtlOutputFormat format) {
    ...
    "stableId": "{:x}"
            ...
            (w->m_workspace ? Workspace::workspaceTypeToString(w->m_workspace->type()) : ""), escapeJSONStrings(!w->m_workspace ? "" : w->m_workspace->displayName()),
        ...
        return std::format(
            ...
            (!w->m_workspace ? "" : w->m_workspace->displayName()), sc<int>(w->isFloating()), w->monitorID(), w->metadata().appID(), w->metadata().title(),
            w->metadata().initialAppID(), w->metadata().initialTitle(), w->backend().pid(), sc<int>(w->backend().isX11()),
...
std::string CCommandFormatter::getWorkspaceData(PHLWORKSPACE w, eHyprCtlOutputFormat format) {
    ...
    const auto  REGULAR    = dynamicPointerCast<Workspace::CRegularWorkspace>(w);
    const bool  PERSISTENT = REGULAR && REGULAR->isPersistent();
    ...
    "tiledLayout": "{}"
                           ...
                           Workspace::workspaceTypeToString(w->type()), escapeJSONStrings(w->displayName()), escapeJSONStrings(PMONITOR ? PMONITOR->m_name : "?"),
        ...
        return std::format("workspace {} ({}) on monitor {}:\n\tmonitorID: {}\n\twindows: {}\n\thasfullscreen: {}\n\tlastwindow: 0x{:x}\n\tlastwindowtitle: {}\n\tispersistent: "
                           "{}\n\ttiledLayout: {}\n\n",
                           w->addressableName(), w->displayName(), PMONITOR ? PMONITOR->m_name : "?", PMONITOR ? std::to_string(PMONITOR->m_id) : "null", w->getWindowCount(),
                           sc<int>(Fullscreen::controller()->hasFullscreen(w)), rc<uintptr_t>(PLASTW.get()), PLASTW ? PLASTW->metadata().title() : "", sc<int>(PERSISTENT),
                           layoutName);
    }
}
...
static std::string getWorkspaceRuleData(const Config::CWorkspaceRule& r, eHyprCtlOutputFormat format) {
    ...
    if (format == eHyprCtlOutputFormat::FORMAT_JSON) {
        ...
        const std::string blur        = sc<bool>(r.m_blur) ? std::format(",\n    \"blur\": {}", boolToString(r.m_blur.value())) : "";
    ...
    "workspaceString": "{}"{}{}{}{}{}{}{}{}{}{}{}{}{}
                                         ...
                                         escapeJSONStrings(r.m_workspaceString), enabled, monitor, default_, persistent, blur, gapsIn, gapsOut, borderSize, border, rounding, decorate,
        ...
        const std::string blur           = std::format("\tblur: {}\n", sc<bool>(r.m_blur) ? boolToString(r.m_blur.value()) : "<unset>");
        const std::string gapsIn         = sc<bool>(r.m_gapsIn) ?
        ...
        std::string result = std::format("Workspace rule {}:\n{}{}{}{}{}{}{}{}{}{}{}{}{}{}\n", escapeJSONStrings(r.m_workspaceString), enabled, monitor, default_, persistent, blur, gapsIn,
...
void IPC::Socket1::refreshState() {
    ...
    for (const auto& window : Desktop::windowState()->windows()) {
        if (!window->mapped() || !window->m_workspace || !window->m_workspace->visible())
            continue;
