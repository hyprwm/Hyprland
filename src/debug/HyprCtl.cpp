...
std::string CHyprCtl::getMonitorData(Hyprutils::Memory::CSharedPointer<CMonitor> m, eHyprCtlOutputFormat format) {
    ...
    "availableModes": [{}]
            ...
            ((uint64_t)m->m_currentTearing.get() ? "true" : "false"), (uint64_t)m->m_currentTearing.get(), ((uint64_t)m->m_currentScanout.get() ? "true" : "false"),
            (uint64_t)m->m_currentScanout.get(), (m->m_enabled ? "false" : "true"), formatToString(m->m_output->state->state().drmFormat),
        ...
        result += std::format(
            ...
            m->activeSpecialWorkspaceID(), (m->m_activeSpecialWorkspace ? m->m_activeSpecialWorkspace->m_name : ""), (int)m->m_reservedTopLeft.x, (int)m->m_reservedTopLeft.y,
            (int)m->m_reservedBottomRight.x, (int)m->m_reservedBottomRight.y, m->m_scale, (int)m->m_transform, (m == g_pCompositor->m_lastMonitor ? "yes" : "no"),
            (int)m->m_dpmsStatus, m->m_output->state->state().adaptiveSync, (uint64_t)m->m_solitaryClient.get(), ((uint64_t)m->m_currentTearing.get() ? "true" : "false"),
            (uint64_t)m->m_currentTearing.get(), ((uint64_t)m->m_currentScanout.get() ? "true" : "false"), (uint64_t)m->m_currentScanout.get(), !m->m_enabled,
