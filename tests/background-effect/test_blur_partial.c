/*
 * Test: Partial blur region via ext-background-effect-v1
 *
 * Creates a centered layer surface and sets a blur region covering only the left half.
 * Verifies that blur is spatially limited to the specified region.
 */
...
int main(void) {
    ...
    struct zwlr_layer_surface_v1* layer_surface = zwlr_layer_shell_v1_get_layer_surface(layer_shell, surface, NULL, ZWLR_LAYER_SHELL_V1_LAYER_TOP, "test-blur-partial");
    ...
    printf("Partial blur test running (left half blurred). Press Ctrl+C to exit.\n");
