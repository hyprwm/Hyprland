...
eFullscreenRequestResult CSpace::setFullscreen(SP<ITarget> t, eFullscreenMode currentEffectiveMode, eFullscreenMode mode) {
    ...
    t->setLayoutManagedFullscreen(REQUEST_RESULT == FULLSCREEN_REQUEST_HANDLED_BY_LAYOUT && (mode == FSMODE_FULLSCREEN || mode == FSMODE_MAXIMIZED)); // ERSTARR -> set maximise window as layoutmanagedFS
