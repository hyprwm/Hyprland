...
void CMonitor::onConnect(bool noRule) {
    ...
    Debug::log(LOG, "Added new monitor with name {} at {:j0} with size {:j0}, pointer {:x}", m_output->name, m_position, m_pixelSize, reinterpret_cast<uintptr_t>(m_output.get()));
...
bool CMonitor::applyMonitorRule(SMonitorRule* pMonitorRule, bool force) {
    ...
    if (RULE->resolution == Vector2D()) {
        ...
        addBest3Modes([](auto const& a, auto const& b) {
            if (std::round(a->refreshRate) > std::round(b->refreshRate))
                return true;
            else if (DELTALESSTHAN(static_cast<float>(a->refreshRate), static_cast<float>(b->refreshRate), 1.F) && a->pixelSize.x > b->pixelSize.x && a->pixelSize.y > b->pixelSize.y)
                return true;
            return false;
        });
    ...
    for (auto const& fmt : formats[static_cast<int>(!RULE->enable10bit)]) {
    ...
    Debug::log(LOG, "Monitor {} data dump: res {:X}@{:.2f}Hz, scale {:.2f}, transform {}, pos {:X}, 10b {}", m_name, m_pixelSize, m_refreshRate, m_scale, static_cast<int>(m_transform),
               m_position, static_cast<int>(m_enabled10bit));
...
void CMonitor::setupDefaultWS(const SMonitorRule& monitorRule) {
    ...
    Debug::log(LOG, "New monitor: WORKSPACEID {}, exists: {}", wsID, static_cast<int>(PNEWWORKSPACE != nullptr));
...
bool CMonitor::attemptDirectScanout() {
    ...
    Debug::log(TRACE, "attemptDirectScanout: surface {:x} passed, will attempt, buffer {}", reinterpret_cast<uintptr_t>(PSURFACE.get()), reinterpret_cast<uintptr_t>(PSURFACE->m_current.buffer.m_buffer.get()));
    ...
    if (m_lastScanout.expired()) {
        ...
        Debug::log(LOG, "Entered a direct scanout to {:x}: \"{}\"", reinterpret_cast<uintptr_t>(PCANDIDATE.get()), PCANDIDATE->m_title);
    }
