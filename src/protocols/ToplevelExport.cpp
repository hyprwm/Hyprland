...
void CToplevelExportFrame::shareFrame(wl_resource* buffer, bool ignoreDamage) {
    LOGM(LOG, "stats: c {}, f {}", PROTO::toplevelExport->m_clients.size(), PROTO::toplevelExport->m_frames.size());
    ...
    LOGM(LOG, "shareFrame: bufSize {}, frameBufSize {}", PBUFFER->size, m_frame->bufferSize());
