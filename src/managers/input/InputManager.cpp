...
void CInputManager::mouseMoveUnified(uint32_t time, bool refocus, bool mouse, std::optional<Vector2D> overridePos) {
    ...
    if (!g_pSeatManager->m_mouse.expired()) {
        ...
        if (isConstrained()) {
            ...
            if (WINDOW) {
                const auto MODE          = WINDOW->m_ruleApplicator->confinePointer().valueOrDefault();
                const bool shouldConfine = MODE == Desktop::Rule::CONFINEPOINTER_ALWAYS || (MODE == Desktop::Rule::CONFINEPOINTER_FULLSCREEN && WINDOW->isFullscreen());
                if (shouldConfine) {
                    const auto BOX = SURF->getSurfaceBoxGlobal();
                    if (BOX.has_value()) {
                        CRegion rg;
