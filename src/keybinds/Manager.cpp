...
static std::optional<Input::eKeyboardModifiers> modifierFromXkb(xkb_keysym_t sym) {
    switch (sym) {
        case XKB_KEY_Super_L:
        case XKB_KEY_Super_R: return Input::HL_MODIFIER_META;
        case XKB_KEY_Alt_L:
        case XKB_KEY_Alt_R: return Input::HL_MODIFIER_ALT;
        case XKB_KEY_Control_L:
        case XKB_KEY_Control_R: return Input::HL_MODIFIER_CTRL;
        case XKB_KEY_Shift_L:
        case XKB_KEY_Shift_R: return Input::HL_MODIFIER_SHIFT;
        case XKB_KEY_Caps_Lock: return Input::HL_MODIFIER_CAPS;
        case XKB_KEY_Num_Lock: return Input::HL_MODIFIER_MOD2;
        default: break;
    }
    return std::nullopt;
}
...
SBindResult CKeybindManager::processEvent(const SBindEventContext& context, const SP<IKeyboard>& keyboard, SPressedInput* pressedInput) {
    ...
    for (const auto& bind : RESOLUTION.partial) {
        ...
        if (context.pressed && (bind->hasFlag(BIND_FLAG_NON_CONSUMING) || bind->hasFlag(BIND_FLAG_RELEASE) || bind->hasFlag(BIND_FLAG_AUTO_CONSUMING)))
            continue;
...
bool CKeybindManager::handleVT(xkb_keysym_t keysym) {
    ...
    const auto VTNR = keysym - XKB_KEY_XF86Switch_VT_1 + 1;

    const auto CUR = g_pCompositor->getVTNr();
    if (CUR && *CUR == VTNR)
        return true;
