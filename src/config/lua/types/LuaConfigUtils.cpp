...
UP<ILuaConfigValue> Lua::fromGenericValue(SP<Config::Values::IValue> v) {
    ...
    if (auto p = dc<Config::Values::CIntValue*>(v.get()))
        return withRefresh(makeUnique<CLuaConfigInt>(p->defaultVal(), p->m_min, p->m_max, p->m_map));
    if (auto p = dc<Config::Values::CFloatValue*>(v.get()))
        return withRefresh(makeUnique<CLuaConfigFloat>(p->defaultVal(), p->m_min, p->m_max));
    ...
    if (auto p = dc<Config::Values::CStringValue*>(v.get()))
        return withRefresh(makeUnique<CLuaConfigString>(p->defaultVal(), p->validator()));
    ...
    if (auto p = dc<Config::Values::CVec2Value*>(v.get()))
        return withRefresh(makeUnique<CLuaConfigVec2>(p->defaultVal(), p->validator()));
    if (auto p = dc<Config::Values::CCssGapValue*>(v.get()))
        return withRefresh(makeUnique<CLuaConfigCssGap>(p->defaultVal().m_top, p->m_min, p->m_max));
