...
std::string CHyprCtl::getMonitorData(Hyprutils::Memory::CSharedPointer<CMonitor> m, eHyprCtlOutputFormat format) {
    ...
    "availableModes": [{}]
            ...
            getSolitaryBlockedReason(m, format), (m->m_tearingState.activelyTearing ? "true" : "false"), (uint64_t)m->m_lastScanout.get(), getDSBlockedReason(m, format),
        ...
        result += std::format(
            ...
            (int)m->m_dpmsStatus, m->m_output->state->state().adaptiveSync, (uint64_t)m->m_solitaryClient.get(), getSolitaryBlockedReason(m, format),
