...
void CGroup::replaceMember(PHLWINDOW oldWindow, PHLWINDOW newWindow, std::optional<Fullscreen::eFullscreenMode> internalMode, bool layoutManaged) {
    ...
    if (HAD_FULLSCREEN)
        Fullscreen::controller()->setInternalFullscreenMode(oldWindow, Fullscreen::FSMODE_NONE, LAYOUT_MANAGED);
    ...
    if (NEW_FS_INTERNAL_MODE != Fullscreen::FSMODE_NONE)
        Fullscreen::controller()->setInternalFullscreenMode(newWindow, Fullscreen::FSMODE_NONE, Fullscreen::controller()->layoutManagedFS(newWindow));
    ...
    *oldWindow->presentation().alpha(WINDOW_ALPHA_LAYOUT) = 1.F;
    ...
    if (HAD_FULLSCREEN) {
        Fullscreen::controller()->setInternalFullscreenMode(newWindow, FS_INTERNAL_MODE, LAYOUT_MANAGED);
