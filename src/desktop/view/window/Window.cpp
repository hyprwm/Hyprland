...
bool CWindow::isAllowedOverFullscreen() const {
    ...
    return m_fullscreenPolicy->effectiveAllowedOverFullscreen({
        ...
        .pinned                = (m_state & WINDOW_STATE_PINNED) != WINDOW_STATE_NONE,
...
bool CWindow::canBeTorn() {
    ...
    return m_ruleApplicator->tearing().valueOr((m_hints & WINDOW_HINT_TEAR) != WINDOW_HINT_NONE) && *PTEARING;
}
...
void CWindow::mapWindow() {
    ...
    auto      setStaticProps = [&]() {
        ...
        if (m_ruleApplicator->static_.noInitialFocus.value_or((m_state & WINDOW_STATE_NO_INITIAL_FOCUS) != WINDOW_STATE_NONE))
            m_state |= WINDOW_STATE_NO_INITIAL_FOCUS;
        else
            m_state &= ~WINDOW_STATE_NO_INITIAL_FOCUS;

        if (m_ruleApplicator->static_.pin.value_or((m_state & WINDOW_STATE_PINNED) != WINDOW_STATE_NONE))
    ...
    if ((m_state & WINDOW_STATE_PINNED) != WINDOW_STATE_NONE && !m_target->floating())
        m_state &= ~WINDOW_STATE_PINNED;
    ...
    if (!WORKSPACEARGS[0].empty()) {
        ...
        if (requestedWorkspaceID != WORKSPACE_INVALID) {
            ...
            if (!workspaceSilent) {
                ...
                else if (PMONITOR->activeWorkspaceID() != requestedWorkspaceID && (m_state & WINDOW_STATE_NO_INITIAL_FOCUS) == WINDOW_STATE_NONE) // NOLINTNEXTLINE
    ...
    if (!m_ruleApplicator->noFocus().valueOrDefault() && (m_state & WINDOW_STATE_NO_INITIAL_FOCUS) == WINDOW_STATE_NONE && (!TRAITS.overrideRedirect || TRAITS.wantsFocus) &&
        !workspaceSilent && !monitorSilent && (!PFORCEFOCUS || PFORCEFOCUS == m_self.lock()) && !g_pInputManager->isConstrained()) {
    ...
    if ((m_state & WINDOW_STATE_NO_INITIAL_FOCUS) == WINDOW_STATE_NONE &&
        (requestedInternalFSMode.has_value() || requestedClientFSMode.has_value() || requestedFSState.has_value())) {
...
void CWindow::unmapWindow() {
    ...
    if (IS_CURRENT_WINDOW_FS) {
        if (SWALLOW_UNMAP_RESULT.transferredInternalFullscreen)
            Fullscreen::controller()->clearClientFullscreenMode(m_self.lock());
        else
    ...
    if (wasLastWindow) {
        ...
        if (candidate != Desktop::focusState()->window() && candidate) {
            ...
            if ((*PEXITRETAINSFS || candidate == nextInGroup) && IS_CURRENT_WINDOW_FS && candidate != RESTORED_SWALLOW_WINDOW)
