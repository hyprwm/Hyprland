...
void CEventLoopManager::onFdReadable(SReadableWaiter* waiter) {
    auto it = std::ranges::find_if(m_vReadableWaiters, [waiter](const SReadableWaiter& w) { return w.fd == waiter->fd && w.source == waiter->source; });
...
void CEventLoopManager::doOnReadable(CFileDescriptor fd, const std::function<void()>& fn) {
    ...
    auto& waiter  = m_vReadableWaiters.emplace_back(nullptr, std::move(fd), fn);
    waiter.source = wl_event_loop_add_fd(g_pEventLoopManager->m_sWayland.loop, waiter.fd.get(), WL_EVENT_READABLE, ::handleWaiterFD, &waiter);
}
