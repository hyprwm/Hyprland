...
CFunctionHook::SInstructionProbe CFunctionHook::getInstructionLenAt(void* start) {
    ...
    size_t insSize   = 0;
    while (true) {
        ud_set_input_buffer(&udis, static_cast<uint8_t*>(start), curOffset);
...
CFunctionHook::SInstructionProbe CFunctionHook::probeMinimumJumpSize(void* start, size_t min) {
    ...
    while (size <= min) {
        ...
        auto probe = getInstructionLenAt(static_cast<uint8_t*>(start) + size);
...
CFunctionHook::SAssembly CFunctionHook::fixInstructionProbeRIPCalls(const SInstructionProbe& probe) {
    ...
    uint64_t currentAddress = reinterpret_cast<uint64_t>(m_source);
    // actually newline + 1
    ...
    for (auto const& len : probe.insSizes) {
        ...
        for (size_t i = 0; i < len; ++i) {
            finalBytes[currentDestinationOffset + i] = *reinterpret_cast<char*>(currentAddress + i);
        }
        ...
        if (code.contains("%rip")) {
            ...
            const uint64_t PREDICTEDRIP = reinterpret_cast<uint64_t>(m_trampolineAddr) + currentDestinationOffset + len;
            ...
            for (int i = len - 4 /* 32-bit */; i > 0; --i) {
                if (*reinterpret_cast<int32_t*>(currentAddress + i) == OFFSET) {
                    ripOffset = i;
                    break;
                }
            }
            ...
            *reinterpret_cast<int32_t*>(&finalBytes[currentDestinationOffset + ripOffset]) = NEWRIPOFFSET;
...
bool CFunctionHook::hook() {
    ...
    m_trampolineAddr               = reinterpret_cast<void*>(g_pFunctionHookSystem->getAddressForTrampo());
    ...
    memcpy(static_cast<uint8_t*>(m_trampolineAddr) + HOOKSIZE, PUSH_RAX, sizeof(PUSH_RAX));                                            // then, pushq %rax
    memcpy(static_cast<uint8_t*>(m_trampolineAddr) + HOOKSIZE + sizeof(PUSH_RAX), ABSOLUTE_JMP_ADDRESS, sizeof(ABSOLUTE_JMP_ADDRESS)); // then, jump to source

    // fixup trampoline addr
    *reinterpret_cast<uint64_t*>(static_cast<uint8_t*>(m_trampolineAddr) + TRAMPOLINE_SIZE - sizeof(ABSOLUTE_JMP_ADDRESS) + ABSOLUTE_JMP_ADDRESS_OFFSET) =
        reinterpret_cast<uint64_t>(static_cast<uint8_t*>(m_source) + sizeof(ABSOLUTE_JMP_ADDRESS));

    // make jump to hk
    const auto     PAGESIZE_VAR = sysconf(_SC_PAGE_SIZE);
    const uint8_t* PROTSTART    = static_cast<uint8_t*>(m_source) - (reinterpret_cast<uint64_t>(m_source) % PAGESIZE_VAR);
    const size_t   PROTLEN      = std::ceil(static_cast<float>(ORIGSIZE + (reinterpret_cast<uint64_t>(m_source) - reinterpret_cast<uint64_t>(PROTSTART))) / static_cast<float>(PAGESIZE_VAR)) * PAGESIZE_VAR;
    ...
    memcpy(static_cast<uint8_t*>(m_source) + sizeof(ABSOLUTE_JMP_ADDRESS), POP_RAX, sizeof(POP_RAX));
    size_t currentOp = sizeof(ABSOLUTE_JMP_ADDRESS) + sizeof(POP_RAX);
    memset(static_cast<uint8_t*>(m_source) + currentOp, NOP, ORIGSIZE - currentOp);

    // fixup jump addr
    *reinterpret_cast<uint64_t*>(static_cast<uint8_t*>(m_source) + ABSOLUTE_JMP_ADDRESS_OFFSET) = reinterpret_cast<uint64_t>(m_destination);
...
bool CFunctionHook::unhook() {
    ...
    // allow write to src
    mprotect(static_cast<uint8_t*>(m_source) - reinterpret_cast<uint64_t>(m_source) % sysconf(_SC_PAGE_SIZE), sysconf(_SC_PAGE_SIZE), PROT_READ | PROT_WRITE | PROT_EXEC);

    // write back original bytes
    memcpy(m_source, m_originalBytes.data(), m_hookLen);

    // revert mprot
    mprotect(static_cast<uint8_t*>(m_source) - reinterpret_cast<uint64_t>(m_source) % sysconf(_SC_PAGE_SIZE), sysconf(_SC_PAGE_SIZE), PROT_READ | PROT_EXEC);
    ...
    if (!page->addr) {
        ...
        for (int attempt = 0; attempt < 2; ++attempt) {
            for (int i = 0; i <= 2; ++i) {
                ...
                page->addr = reinterpret_cast<uint64_t>(mmap(reinterpret_cast<void*>(PAGEADDR), PAGESIZE_VAR, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0));
                page->len  = PAGESIZE_VAR;
                page->used = 0;

                Debug::log(LOG, "Attempted to allocate 0x{:x}, got 0x{:x}", PAGEADDR, page->addr);

                if (page->addr == reinterpret_cast<uint64_t>(MAP_FAILED))
                    continue;
                if (page->addr != PAGEADDR && attempt == 0) {
                    munmap(reinterpret_cast<void*>(page->addr), PAGESIZE_VAR);
                    page->addr = 0;
                    page->len  = 0;
                    continue;
                }
