...
#define MS makeConfigValue
    ...
    return std::vector<SP<IValue>>{
        ...
        MS<Bool>("group:groupbar:keep_outer_gap", "render the gap between the groupbar and the outer border", true),
        MS<Int>("group:groupbar:position", "sets the position of the groupbar.", 0, {.min = 0, .max = 1, .map = OptionMap{{"top", 0}, {"bottom", 1}}}),
