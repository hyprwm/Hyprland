...
void CX11DataDevice::sendDndEvent(xcb_window_t targetWindow, xcb_atom_t type, xcb_client_message_data_t& data) {
    ...
    xcb_send_event(conn, 0, targetWindow, XCB_EVENT_MASK_NO_EVENT, reinterpret_cast<const char*>(&event));
    xcb_flush(conn);
}
...
xcb_window_t CX11DataDevice::getProxyWindow(xcb_window_t window) {
    ...
    if (isValidPropertyReply(proxyReply)) {
        xcb_window_t              proxyWindow = *static_cast<xcb_window_t*>(xcb_get_property_value(proxyReply));
        ...
        if (isValidPropertyReply(proxyVerifyReply)) {
            xcb_window_t verifyWindow = *static_cast<xcb_window_t*>(xcb_get_property_value(proxyVerifyReply));
...
void CX11DataDevice::sendMotion(uint32_t timeMs, const Vector2D& local) {
    ...
    const uint32_t            coords  = (static_cast<uint32_t>(XCOORDS.x) << 16) | static_cast<uint32_t>(XCOORDS.y);
