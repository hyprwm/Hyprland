...
eDynamicPermissionAllowMode CDynamicPermissionManager::clientPermissionMode(wl_client* client, eDynamicPermissionType permission) {
    ...
    Debug::log(TRACE, "CDynamicPermissionManager::clientHasPermission: checking permission {} for client {:x} (binary {})", permissionToString(permission), reinterpret_cast<uintptr_t>(client),
...
eDynamicPermissionAllowMode CDynamicPermissionManager::clientPermissionModeWithString(pid_t pid, const std::string& str, eDynamicPermissionType permission) {
    ...
    if (pid > 0) {
        ...
        binaryName = specialPidToString(static_cast<eSpecialPidTypes>(pid));
...
void CDynamicPermissionManager::askForPermission(wl_client* client, const std::string& binaryPath, eDynamicPermissionType type, pid_t pid) {
    ...
    if (binaryPath.empty())
        description = std::format(std::runtime_format(permissionToHumanString(type)), std::format("unknown application (wayland client ID 0x{:x})", reinterpret_cast<uintptr_t>(client)));
    else if (client) {
        ...
        std::string lookup = "";
        if (pid < 0)
            lookup = specialPidToString(static_cast<eSpecialPidTypes>(pid));
        else {
