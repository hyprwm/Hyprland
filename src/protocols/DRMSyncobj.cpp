...
CDRMSyncobjSurfaceResource::CDRMSyncobjSurfaceResource(UP<CWpLinuxDrmSyncobjSurfaceV1>&& resource_, SP<CWLSurfaceResource> surface_) :
    ...
    listeners.surfacePrecommit = surface->events.precommit.registerListener([this](std::any d) {
        ...
        if (pendingAcquire.timeline().expired() != pendingRelease.timeline().expired()) {
            resource->error(pendingAcquire.timeline().expired() ? WP_LINUX_DRM_SYNCOBJ_SURFACE_V1_ERROR_NO_ACQUIRE_POINT : WP_LINUX_DRM_SYNCOBJ_SURFACE_V1_ERROR_NO_RELEASE_POINT,
                            "Missing timeline");
            surface->pending.rejected = true;
            return;
        }
