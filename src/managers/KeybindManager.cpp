...
void CKeybindManager::moveActiveToWorkspaceSilent(std::string args) {
    ...
    if (PWINDOW == g_pCompositor->m_pLastWindow) {
        if (const auto PATCOORDS = g_pCompositor->vectorToWindowUnified(OLDMIDDLE, FULL_EXTENTS | ALLOW_FLOATING, PWINDOW); PATCOORDS)
...
void CKeybindManager::mouse(std::string args) {
    ...
    if (ARGS[0] == "movewindow") {
        if (PRESSED) {
            ...
            CWindow*   pWindow     = g_pCompositor->vectorToWindowUnified(mouseCoords, FULL_EXTENTS | ALLOW_FLOATING);
        ...
        if (PRESSED) {
            ...
            g_pInputManager->currentlyDraggedWindow = g_pCompositor->vectorToWindowUnified(g_pInputManager->getMouseCoordsInternal(), FULL_EXTENTS | ALLOW_FLOATING);
