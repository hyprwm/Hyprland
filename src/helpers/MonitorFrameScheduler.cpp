...
void CMonitorFrameScheduler::onSyncFired() {
    ...
    Debug::log(TRACE, "CMonitorFrameScheduler: {} -> onSyncFired, rendering third frame.", m_monitor->m_name);
...
void CMonitorFrameScheduler::onFinishRender() {
    if (!m_monitor->m_inFence.isValid()) {
        Debug::log(ERR, "CMonitorFrameScheduler: {} -> onFinishRender, m_inFence is not valid.", m_monitor->m_name);
        return;
    }
