...
CSpace::CSpace(PHLWORKSPACE parent) : m_parent(parent) {
    ...
    m_geomUpdateCallback = Event::bus()->m_events.monitor.layoutChanged.listen([this] {
        ...
        if (m_algorithm)
            m_algorithm->recalculate(RECALCULATE_REASON_EVENT_LOOP_DO_LATER);
    });
}
...
eFullscreenRequestResult CSpace::setFullscreen(SP<ITarget> t, eFullscreenMode currentEffectiveMode, eFullscreenMode mode) {
    ...
    t->setLayoutManagedFullscreen(REQUEST_RESULT == FULLSCREEN_REQUEST_HANDLED_BY_LAYOUT &&
                                  (mode == FSMODE_FULLSCREEN || mode == FSMODE_MAXIMIZED));
...
bool Layout::isHardRecalculateReason(eRecalculateReason reason) {
    return reason != RECALCULATE_REASON_EVENT_LOOP_DO_LATER && reason != RECALCULATE_REASON_WORKSPACE_CHANGE && reason != RECALCULATE_REASON_SPECIAL_WORKSPACE_TOGGLE &&
...
eRecalculateReason Layout::recalcMonitorReasonToRecalcReason(CLayoutManager::eRecalculateMonitorReason reason) {
    ...
    switch (reason) {
        ...
        case CLayoutManager::RECALCULATE_MONITOR_REASON_EVENT_LOOP_DO_LATER: return RECALCULATE_REASON_EVENT_LOOP_DO_LATER;
