...
void CXWM::handleCreate(xcb_create_notify_event_t* e) {
    ...
    XSURF->m_self    = XSURF;
    Debug::log(LOG, "[xwm] New XSurface at {:x} with xid of {}", reinterpret_cast<uintptr_t>(XSURF.get()), e->window);

    const auto WINDOW = CWindow::create(XSURF);
    g_pCompositor->m_windows.emplace_back(WINDOW);
    WINDOW->m_self = WINDOW;
    Debug::log(LOG, "[xwm] New XWayland window at {:x} for surf {:x}", reinterpret_cast<uintptr_t>(WINDOW.get()), reinterpret_cast<uintptr_t>(XSURF.get()));
}
...
void CXWM::readProp(SP<CXWaylandSurface> XSURF, uint32_t atom, xcb_get_property_reply_t* reply) {
    ...
    const auto* value    = static_cast<const char*>(xcb_get_property_value(reply));
...
void CXWM::handleClientMessage(xcb_client_message_event_t* e) {
    ...
    if (e->type == HYPRATOMS["WM_PROTOCOLS"]) {
        ...
        XSURF->m_wlSerial   = (static_cast<uint64_t>(serialHigh) << 32) | serialLow;

        Debug::log(LOG, "[xwm] surface {:x} requests serial {:x}", reinterpret_cast<uintptr_t>(XSURF.get()), XSURF->m_wlSerial);
...
void CXWM::sendWMMessage(SP<CXWaylandSurface> surf, xcb_client_message_data_t* data, uint32_t mask) {
    ...
    xcb_send_event(getConnection(), 0, surf->m_xID, mask, reinterpret_cast<const char*>(&event));
    xcb_flush(getConnection());
}
...
void CXWM::selectionSendNotify(xcb_selection_request_event_t* e, bool success) {
    xcb_selection_notify_event_t selection_notify = {
        ...
        .target        = e->target,
        .property      = success ? e->property : static_cast<uint32_t>(XCB_ATOM_NONE),
    };

    xcb_send_event(getConnection(), 0, e->requestor, XCB_EVENT_MASK_NO_EVENT, reinterpret_cast<const char*>(&selection_notify));
    xcb_flush(getConnection());
}
...
bool CXWM::handleSelectionEvent(xcb_generic_event_t* e) {
    switch (e->response_type & XCB_EVENT_RESPONSE_TYPE_MASK) {
        ...
        case XCB_PROPERTY_NOTIFY: {
            return handleSelectionPropertyNotify(reinterpret_cast<xcb_property_notify_event_t*>(e));
        }
        case XCB_SELECTION_REQUEST: {
            handleSelectionRequest(reinterpret_cast<xcb_selection_request_event_t*>(e));
            return true;
        }
    }

    if (e->response_type - m_xfixes->first_event == XCB_XFIXES_SELECTION_NOTIFY)
        return handleSelectionXFixesNotify(reinterpret_cast<xcb_xfixes_selection_notify_event_t*>(e));
...
int CXWM::onEvent(int fd, uint32_t mask) {
    ...
    while (true) {
        ...
        switch (event->response_type & XCB_EVENT_RESPONSE_TYPE_MASK) {
            case XCB_CREATE_NOTIFY: handleCreate(reinterpret_cast<xcb_create_notify_event_t*>(event.get())); break;
            case XCB_DESTROY_NOTIFY: handleDestroy(reinterpret_cast<xcb_destroy_notify_event_t*>(event.get())); break;
            case XCB_CONFIGURE_REQUEST: handleConfigureRequest(reinterpret_cast<xcb_configure_request_event_t*>(event.get())); break;
            case XCB_CONFIGURE_NOTIFY: handleConfigureNotify(reinterpret_cast<xcb_configure_notify_event_t*>(event.get())); break;
            case XCB_MAP_REQUEST: handleMapRequest(reinterpret_cast<xcb_map_request_event_t*>(event.get())); break;
            case XCB_MAP_NOTIFY: handleMapNotify(reinterpret_cast<xcb_map_notify_event_t*>(event.get())); break;
            case XCB_UNMAP_NOTIFY: handleUnmapNotify(reinterpret_cast<xcb_unmap_notify_event_t*>(event.get())); break;
            case XCB_PROPERTY_NOTIFY: handlePropertyNotify(reinterpret_cast<xcb_property_notify_event_t*>(event.get())); break;
            case XCB_CLIENT_MESSAGE: handleClientMessage(reinterpret_cast<xcb_client_message_event_t*>(event.get())); break;
            case XCB_FOCUS_IN: handleFocusIn(reinterpret_cast<xcb_focus_in_event_t*>(event.get())); break;
            case XCB_FOCUS_OUT: handleFocusOut(reinterpret_cast<xcb_focus_out_event_t*>(event.get())); break;
            case 0: handleError(reinterpret_cast<xcb_value_error_t*>(event.get())); break;
            default: {
...
void CXWM::activateSurface(SP<CXWaylandSurface> surf, bool activate) {
    ...
    if (!surf || (!activate && g_pCompositor->m_lastWindow && !g_pCompositor->m_lastWindow->m_isX11)) {
        ...
        setActiveWindow(surf ? surf->m_xID : static_cast<uint32_t>(XCB_WINDOW_NONE));
        focusWindow(surf);
    }
...
void CXWM::onNewSurface(SP<CWLSurfaceResource> surf) {
    ...
    Debug::log(LOG, "[xwm] New XWayland surface at {:x}", reinterpret_cast<uintptr_t>(surf.get()));
...
void CXWM::onNewResource(SP<CXWaylandSurfaceResource> resource) {
    Debug::log(LOG, "[xwm] New XWayland resource at {:x}", reinterpret_cast<uintptr_t>(resource.get()));
...
void CXWM::associate(SP<CXWaylandSurface> surf, SP<CWLSurfaceResource> wlSurf) {
    ...
    if (existing != m_surfaces.end()) {
        Debug::log(WARN, "[xwm] associate() called but surface is already associated to {:x}, ignoring...", reinterpret_cast<uintptr_t>(surf.get()));
        return;
    }
...
void CXWM::dissociate(SP<CXWaylandSurface> surf) {
    ...
    Debug::log(LOG, "Dissociate for {:x}", reinterpret_cast<uintptr_t>(surf.get()));
}
...
void CXWM::setClipboardToWayland(SXSelection& sel) {
    ...
    Debug::log(LOG, "[xwm] X selection at {:x} takes {}", reinterpret_cast<uintptr_t>(sel.dataSource.get()), (&sel == &m_clipboard) ? "clipboard" : "primary selection");
...
static int writeDataSource(int fd, uint32_t mask, void* data) {
    auto selection = static_cast<SXSelection*>(data);
...
int SXSelection::onWrite() {
    ...
    auto&   transfer  = *it;
    char*   property  = static_cast<char*>(xcb_get_property_value(transfer->propertyReply));
