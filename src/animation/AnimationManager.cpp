...
static void preDamageWorkspace(PHLWORKSPACE pWorkspace, PHLMONITOR pMonitor) {
    ...
    for (auto const& w : Desktop::windowState()->windows()) {
        ...
        if (w->isFloating() && (w->m_state & Desktop::View::WINDOW_STATE_PINNED) == Desktop::View::WINDOW_STATE_NONE) {
    ...
    for (auto const& w : Desktop::windowState()->windows()) {
        if (!validMapped(w) || w->m_workspace != pWorkspace || (w->m_state & Desktop::View::WINDOW_STATE_PINNED) != Desktop::View::WINDOW_STATE_NONE)
            continue;
...
void CHyprAnimationManager::tick() {
    ...
    for (const auto& owner : owners) {
        if (owner.window)
            ...
            if (owner.entire) {
                for (auto const& w : Desktop::windowState()->windows()) {
                    if (!validMapped(w) || w->m_workspace != owner.workspace || (w->m_state & Desktop::View::WINDOW_STATE_PINNED) != Desktop::View::WINDOW_STATE_NONE)
                        continue;
