...
void CHyprMasterLayout::onWindowRemovedTiling(CWindow* pWindow) {
    ...
    const bool SMALL_SPLIT = g_pConfigManager->getConfigValuePtr("master:allow_small_split")->intValue == 1;
    ...
    if (PNODE->isMaster && (MASTERSLEFT <= 1 || SMALL_SPLIT)) {
...
void CHyprMasterLayout::calculateWorkspace(const int& ws) {
    ...
    const auto STACK_WINDOWS = WINDOWS - MASTERS;
    //compute placement of master window(s)
    if ((WINDOWS < 2 || STACK_WINDOWS == 0) && !centerMasterWindow) {
        ...
        return;
    } else if (orientation == ORIENTATION_LEFT || orientation == ORIENTATION_RIGHT || (orientation == ORIENTATION_CENTER && STACK_WINDOWS <= 1)) {
    ...
    } else if (orientation == ORIENTATION_CENTER && STACK_WINDOWS >= 2) {
        float       heightLeft = PMONITOR->vecSize.y - PMONITOR->vecReservedBottomRight.y - PMONITOR->vecReservedTopLeft.y;
        int         nodesLeft  = MASTERS;
        float       nextY      = 0;
    ...
    if (orientation == ORIENTATION_LEFT || orientation == ORIENTATION_RIGHT || (orientation == ORIENTATION_CENTER && STACK_WINDOWS <= 1)) {
        float       heightLeft = PMONITOR->vecSize.y - PMONITOR->vecReservedBottomRight.y - PMONITOR->vecReservedTopLeft.y;
        float       nextY      = 0;
        ...
        for (auto& nd : m_lMasterNodesData) {
            ...
            if (orientation == ORIENTATION_LEFT || (orientation == ORIENTATION_CENTER && STACK_WINDOWS <= 1)) {
...
std::any CHyprMasterLayout::layoutMessage(SLayoutMessageHeader header, std::string message) {
    ...
    // * auto (default) - swap the focus with the first child, if the current focus was master, otherwise focus master
        ...
        bool       SMALL_SPLIT = g_pConfigManager->getConfigValuePtr("master:allow_small_split")->intValue == 1;
