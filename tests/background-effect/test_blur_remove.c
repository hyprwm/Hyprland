/*
 * Test: Set then remove blur via ext-background-effect-v1
 *
 * Creates a centered layer surface, sets a blur region, waits 3 seconds,
 * then sets NULL region to remove blur. Uses a proper event loop so the
 * surface is visible throughout.
 */
...
int main(void) {
    ...
    struct zwlr_layer_surface_v1* layer_surface = zwlr_layer_shell_v1_get_layer_surface(layer_shell, surface, NULL, ZWLR_LAYER_SHELL_V1_LAYER_TOP, "test-blur-remove");
    ...
    printf("Blur enabled. Removing in 3 seconds...\n");
    ...
    while (running) {
        ...
        if (!removed && elapsed >= 3.0) {
            ext_background_effect_surface_v1_set_blur_region(bg_effect, NULL);
            wl_surface_commit(surface);
            printf("Blur removed. Exiting in 5 seconds...\n");
            removed = 1;
        }

        if (removed && elapsed >= 8.0)
            break;
