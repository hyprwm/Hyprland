...
int CWorkspace::getWindowCount(std::optional<bool> onlyTiled, std::optional<bool> onlyPinned, std::optional<bool> onlyVisible) {
    ...
    for (auto const& t : m_space->targets()) {
        ...
        if (onlyTiled.has_value() && t->floating() == onlyTiled.value())
            continue;
        if (onlyPinned.has_value() && (!t->window() || ((t->window()->m_state & WINDOW_STATE_PINNED) != WINDOW_STATE_NONE) != onlyPinned.value()))
            continue;
...
int CWorkspace::getGroups(std::optional<bool> onlyTiled, std::optional<bool> onlyPinned, std::optional<bool> onlyVisible) {
    ...
    for (auto const& g : Desktop::View::groups()) {
        ...
        if (onlyTiled.has_value() && HEAD->isFloating() == onlyTiled.value())
            continue;
        if (onlyPinned.has_value() && ((HEAD->m_state & WINDOW_STATE_PINNED) != WINDOW_STATE_NONE) != onlyPinned.value())
            continue;
...
bool CWorkspace::hasUrgentWindow() {
    return std::ranges::any_of(Desktop::windowState()->windows(),
                               [this](const auto& w) { return w->m_workspace == m_self && w->mapped() && (w->m_hints & WINDOW_HINT_URGENT) != WINDOW_HINT_NONE; });
}
