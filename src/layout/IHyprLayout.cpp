...
void IHyprLayout::performSnap(Vector2D& sourcePos, Vector2D& sourceSize, PHLWINDOW DRAGGINGWINDOW, const eMouseBindMode MODE, const int CORNER, const Vector2D& BEGINSIZE) {
    ...
    if (*SNAPWINDOWGAP) {
        ...
        const auto*  GAPSIN = *SNAPRESPECTGAPS ? static_cast<CCssGapData*>(PGAPSIN.ptr()->getData()) : &GAPSNONE;
    ...
    if (*SNAPMONITORGAP) {
        ...
        const auto*  GAPSOUT    = *SNAPRESPECTGAPS ? static_cast<CCssGapData*>(PGAPSOUT.ptr()->getData()) : &GAPSNONE;
...
void IHyprLayout::changeWindowFloatingMode(PHLWINDOW pWindow) {
    ...
    // event
    g_pEventManager->postEvent(SHyprIPCEvent{"changefloatingmode", std::format("{:x},{}", reinterpret_cast<uintptr_t>(pWindow.get()), static_cast<int>(TILED))});
