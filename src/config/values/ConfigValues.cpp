...
#define MS makeConfigValue
    ...
    return std::vector<SP<IValue>>{
        ...
        MS<Int>("misc:exit_window_retains_fullscreen", "whether closing a fullscreen window makes the next focused window fullscreen", 0,
                {.min = 0, .max = 3, .map = OptionMap{{"disabled", 0}, {"enabled", 1}, {"only when closing a grouped window", 2}, {"only when closing a non-grouped window", 3}}}),
