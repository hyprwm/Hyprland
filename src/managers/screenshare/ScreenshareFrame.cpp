...
eScreenshareError CScreenshareFrame::share(SP<IHLBuffer> buffer, const CRegion& clientDamage, FScreenshareCallback callback) {
    ...
    m_damage.transform(wlTransformToHyprutils(PMONITOR->m_transform), PMONITOR->m_pixelSize.x, PMONITOR->m_pixelSize.y);
...
bool CScreenshareFrame::copyShm() {
    ...
    uint32_t packStride = NFormatUtils::minStride(PFORMAT, m_session->m_box.w);
