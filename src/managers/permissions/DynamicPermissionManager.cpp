...
static const char* permissionToString(eDynamicPermissionType type) {
    switch (type) {
        ...
        case PERMISSION_TYPE_CURSOR: return "PERMISSION_TYPE_CURSOR_POS";
    }
...
static const char* permissionToHumanString(eDynamicPermissionType type) {
    switch (type) {
        ...
        case PERMISSION_TYPE_CURSOR:
            return "An application <b>{}</b> is trying to capture the position of the cursor when it's inside the application.<br/><br/>Do you want to allow it to do so?";
    }
