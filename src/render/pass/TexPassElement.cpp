...
void CTexPassElement::draw(const CRegion& damage) {
    ...
    g_pHyprOpenGL->renderTextureInternalWithDamage(data.tex, &data.box, data.a, data.damage.empty() ? damage : data.damage, data.round, data.roundingPower, data.syncTimeline, data.syncPoint);
    if (data.replaceProjection)
