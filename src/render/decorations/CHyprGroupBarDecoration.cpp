...
void CHyprGroupBarDecoration::draw(PHLMONITOR pMonitor, float const& a) {
    ...
    auto* const GROUPCOLACTIVE             = static_cast<CGradientValueData*>((PGROUPCOLACTIVE.ptr())->getData());
    auto* const GROUPCOLINACTIVE           = static_cast<CGradientValueData*>((PGROUPCOLINACTIVE.ptr())->getData());
    auto* const GROUPCOLACTIVELOCKED       = static_cast<CGradientValueData*>((PGROUPCOLACTIVELOCKED.ptr())->getData());
    auto* const GROUPCOLINACTIVELOCKED     = static_cast<CGradientValueData*>((PGROUPCOLINACTIVELOCKED.ptr())->getData());
...
CTitleTex::CTitleTex(PHLWINDOW pWindow, const Vector2D& bufferSize, const float monitorScale) : m_content(pWindow->m_title), m_windowOwner(pWindow) {
    ...
    const auto       FONTWEIGHTACTIVE   = static_cast<CFontWeightConfigValueData*>((PTITLEFONTWEIGHTACTIVE.ptr())->getData());
    const auto       FONTWEIGHTINACTIVE = static_cast<CFontWeightConfigValueData*>((PTITLEFONTWEIGHTINACTIVE.ptr())->getData());
...
static void renderGradientTo(SP<CTexture> tex, CGradientValueData* grad) {
    ...
    for (unsigned long i = 0; i < grad->m_colors.size(); i++) {
        cairo_pattern_add_color_stop_rgba(pattern, 1 - static_cast<double>(i + 1) / (grad->m_colors.size() + 1), grad->m_colors[i].r, grad->m_colors[i].g, grad->m_colors[i].b,
                                          grad->m_colors[i].a);
    }
