...
static std::string getWorkspaceData(CWorkspace* w, HyprCtl::eHyprCtlOutputFormat format) {
    ...
    "persistent": {}
                           ...
                           (w->m_bIndestructible ? "true" : "false"));
    } else {
        return std::format("workspace ID {} ({}) on monitor {}:\n\twindows: {}\n\thasfullscreen: {}\n\tlastwindow: 0x{:x}\n\tlastwindowtitle: {}\n\tpersistent: {}\n\n", w->m_iID,
                           w->m_szName, PMONITOR ? PMONITOR->szName : "?", g_pCompositor->getWindowsOnWorkspace(w->m_iID), (int)w->m_bHasFullscreenWindow, (uintptr_t)PLASTW,
                           PLASTW ? PLASTW->m_szTitle : "", (w->m_bIndestructible ? "true" : "false"));
    }
}
