...
static void testSwapWindow() {
    ...
    NLog::log("{}Testing swapwindow with different workspace. Expecting to fail", Colors::YELLOW);
    {
        getFromSocket("/dispatch focuswindow class:kitty_B");
        auto addr = getWindowAddress(getFromSocket("/activewindow"));
        ...
        auto str = getFromSocket(std::format("/dispatch swapwindow address:0x{}", addr));
        EXPECT_CONTAINS(str, std::format("Can't swap with address:0x{}, is in different workspace", addr));
    }
