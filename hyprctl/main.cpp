...
namespace {
    ...
    std::vector<SInstanceData> instances() {
        ...
        std::ranges::sort(result.begin(), result.end(), [&](const auto& a, const auto& b) { return a.time < b.time; });
    ...
    int request(std::string arg, int minArgs = 0, bool needRoll = false) {
        ...
        [[maybe_unused]] const std::string USERID = std::to_string(getUID());
    ...
    int requestIPC(std::string filename, std::string arg) {
        ...
        socketPath << getRuntimeDir() << "/" << instanceSignature << "/" << filename;

        strncpy(serverAddress.sun_path, socketPath.str().c_str(), sizeof(serverAddress.sun_path) - 1);

        if (connect(SERVERSOCKET, (sockaddr*)&serverAddress, SUN_LEN(&serverAddress)) < 0) {
            log("Couldn't connect to " + socketPath.str() + ". (3)");
            return 3;
        }
...
{{
    ...
    int main(int argc, char** argv) {
        ...
        fullRequest = std::string(fullArgs).append("/").append(fullRequest);
