...
void CXWaylandSurface::map() {
    ...
    Debug::log(LOG, "XWayland surface {:x} mapping", reinterpret_cast<uintptr_t>(this));
...
void CXWaylandSurface::unmap() {
    ...
    Debug::log(LOG, "XWayland surface {:x} unmapping", reinterpret_cast<uintptr_t>(this));
...
void CXWaylandSurface::configure(const CBox& box) {
    ...
    if (m_geometry.width == box.width && m_geometry.height == box.height) {
        ...
        xcb_send_event(g_pXWayland->m_wm->getConnection(), false, m_xID, XCB_EVENT_MASK_STRUCTURE_NOTIFY, reinterpret_cast<const char*>(&e));
    }
