...
CConfigManager::CConfigManager() {
    ...
    for (const auto& v : Values::CONFIG_VALUES) {
        ...
        if (auto p = dynamic_cast<Config::Values::CIntValue*>(v.get()))
            registerConfigVar(NAME, Hyprlang::INT{p->defaultVal()});
        else if (auto p = dynamic_cast<Config::Values::CFloatValue*>(v.get()))
            registerConfigVar(NAME, Hyprlang::FLOAT{p->defaultVal()});
        else if (auto p = dynamic_cast<Config::Values::CBoolValue*>(v.get()))
            registerConfigVar(NAME, Hyprlang::INT{p->defaultVal() ? 1 : 0});
        else if (auto p = dynamic_cast<Config::Values::CStringValue*>(v.get()))
            registerConfigVar(NAME, Hyprlang::STRING{p->defaultVal().c_str()});
        else if (auto p = dynamic_cast<Config::Values::CColorValue*>(v.get()))
            registerConfigVar(NAME, Hyprlang::INT{p->defaultVal()});
        else if (auto p = dynamic_cast<Config::Values::CVec2Value*>(v.get()))
            registerConfigVar(NAME, Hyprlang::VEC2{p->defaultVal().x, p->defaultVal().y});
        else if (auto p = dynamic_cast<Config::Values::CCssGapValue*>(v.get()))
            registerConfigVar(NAME, Hyprlang::CUSTOMTYPE{configHandleGapSet, configHandleGapDestroy, std::to_string(p->defaultVal().m_top).c_str()});
        else if (auto p = dynamic_cast<Config::Values::CFontWeightValue*>(v.get()))
            registerConfigVar(NAME, Hyprlang::CUSTOMTYPE{configHandleFontWeightSet, configHandleFontWeightDestroy, std::to_string(p->defaultVal().m_value).c_str()});
        else if (auto p = dynamic_cast<Config::Values::CGradientValue*>(v.get()))
            registerConfigVar(NAME, Hyprlang::CUSTOMTYPE{configHandleGradientSet, configHandleGradientDestroy, "0x000000"});
    }
