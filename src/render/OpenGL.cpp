...
void CHyprOpenGLImpl::begin(PHLMONITOR pMonitor, const CRegion& damage_, SP<IFramebuffer> fb, std::optional<CRegion> finalDamage) {
    ...
    g_pHyprRenderer->m_renderData.mainFB = g_pHyprRenderer->m_renderData.currentFB;
    g_pHyprRenderer->m_renderData.outFB  = fb ? fb : m_renderData.m_currentRenderbuffer->getFB();
...
WP<CShader> CHyprOpenGLImpl::renderToOutputInternal() {
    ...
    WP<CShader>       shader =
        g_pHyprRenderer->m_crashingInProgress ? m_shaders->frag[SH_FRAG_GLITCH] : (m_finalScreenShader->program() ? m_finalScreenShader : m_shaders->frag[SH_FRAG_PASSTHRURGBA]);
