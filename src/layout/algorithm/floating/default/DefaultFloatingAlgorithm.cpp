...
void CDefaultFloatingAlgorithm::newTarget(SP<ITarget> target) {
    ...
    if (target->window() && (target->window()->m_state & Desktop::View::WINDOW_STATE_FIRST_MAP) != Desktop::View::WINDOW_STATE_NONE) {
...
void CDefaultFloatingAlgorithm::movedTarget(SP<ITarget> target, std::optional<Vector2D> focalPoint) {
    if (target->window() && (target->window()->m_state & Desktop::View::WINDOW_STATE_PINNED) != Desktop::View::WINDOW_STATE_NONE) {
