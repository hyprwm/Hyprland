...
SDispatchResult CKeybindManager::swapActive(std::string args) {
    ...
    if (PLASTWINDOW->m_workspace != PWINDOWTOCHANGETO->m_workspace) {
        Debug::log(ERR, "Can't swap with {}, is in different workspace", args);
        return {.success = false, .error = std::format("Can't swap with {}, is in different workspace", args)};
    }
