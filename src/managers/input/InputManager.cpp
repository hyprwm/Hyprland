...
void CInputManager::mouseMoveUnified(uint32_t time, bool refocus, bool mouse, std::optional<Vector2D> overridePos) {
    ...
    if (pFoundWindow) {
        ...
        if (FOLLOWMOUSE != 1 && !refocus) {
            ...
            const bool shouldGiveX11PointerFocus = pFoundWindow && (pFoundWindow->isX11Popup() || pFoundWindow->isX11OverrideRedirect()) && Desktop::focusState()->window() &&
                Desktop::focusState()->window()->m_isX11;
#else
