...
void CScreenshareManager::onOutputCommit(PHLMONITOR monitor) {
    ...
    LOGM(LOG, "stats: s {}, ms {}, f {}", m_sessions.size(), m_managedSessions.size(), m_frames.size());
...
void CScreenshareManager::onTick() {
    ...
    std::ranges::for_each(m_managedSessions, [this](auto& session) {
        ...
        } else if (session->m_framesInLastHalfSecond < 4 && session->m_sentScreencast && LASTFRAMEDELTA > 1.0 && !FRAMEAWAITING) {
...
void CScreenshareManager::screenshareEvents(WP<CScreenshareSession> session, bool started) {
    if (session.expired()) {
        LOGM(LOG, "screenshareEvents: FAILED");
        return;
    }
    ...
    if (started) {
        ...
        LOGM(LOG, "Stopped screenshare session for ({}): {}", session->m_type, session->m_name);
    }
}
