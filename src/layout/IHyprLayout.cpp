    ...
    if (DRAGGINGWINDOW->m_bDraggingTiled) {
        ...
        if (*PPRECISEMOUSE) {
            ...
            if (pReferenceWindow && pReferenceWindow != DRAGGINGWINDOW) {
                const Vector2D draggedCenter   = DRAGGINGWINDOW->m_vRealPosition->goal() + DRAGGINGWINDOW->m_vRealSize->goal() / 2.f;
                const Vector2D referenceCenter = pReferenceWindow->m_vRealPosition->goal() + pReferenceWindow->m_vRealSize->goal() / 2.f;
