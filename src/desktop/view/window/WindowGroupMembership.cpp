...
bool CWindowGroupMembership::canBeGroupedInto(const SP<CGroup>& group) const {
    ...
    return !Desktop::windowState()->groupsLocked()                                  // global group lock disengaged
        && ((m_rules & GROUP_INVADE && (m_window.m_state & WINDOW_STATE_FIRST_MAP)) // window ignores local group locks, or
        ...
        && !(m_rules & GROUP_BARRED && (m_window.m_state & WINDOW_STATE_FIRST_MAP)) // group rule doesn't prevent adding window
