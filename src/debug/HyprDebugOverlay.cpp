...
void CHyprMonitorDebugOverlay::renderData(PHLMONITOR pMonitor, float durationUs) {
    ...
    if (m_lastRenderTimes.size() > static_cast<long unsigned int>(pMonitor->m_refreshRate))
...
void CHyprMonitorDebugOverlay::renderDataNoOverlay(PHLMONITOR pMonitor, float durationUs) {
    ...
    if (m_lastRenderTimesNoOverlay.size() > static_cast<long unsigned int>(pMonitor->m_refreshRate))
...
void CHyprMonitorDebugOverlay::frameData(PHLMONITOR pMonitor) {
    ...
    if (m_lastFrametimes.size() > static_cast<long unsigned int>(pMonitor->m_refreshRate))
    ...
    if (PMONITORFORTICKS == pMonitor) {
        if (m_lastAnimationTicks.size() > static_cast<long unsigned int>(PMONITORFORTICKS->m_refreshRate))
...
int CHyprMonitorDebugOverlay::draw(int offset) {
    ...
    text = std::format("{} FPS", static_cast<int>(FPS));
    ...
    m_lastDrawnBox = {static_cast<int>(g_pCompositor->m_monitors.front()->m_position.x) + MARGIN_LEFT - 1, static_cast<int>(g_pCompositor->m_monitors.front()->m_position.y) + offset + MARGIN_TOP - 1,
                      static_cast<int>(maxTextW) + 2, posY - offset - MARGIN_TOP + 2};
    g_pHyprRenderer->damageBox(m_lastDrawnBox);
